...
> See [references/lobby-management.md](references/lobby-management.md) for the full GDScript lobby implementation (player_list dict, max-players cap, ready toggle RPC, broadcast pattern).
...
> See [references/match-flow.md](references/match-flow.md) for the full state machine with countdown/results timers and GDScript implementations.
...
> See [references/server-config.md](references/server-config.md) for the GDScript argument-parsing helper that reads all four flags safely at startup.

---

## 6. Deployment

A Linux VPS with a `Dockerfile` and `systemd` service file is the standard production layout. The Dockerfile bundles the headless export template, the exported PCK. systemd handles auto-restart, log rotation via `journald`, and resource limits.
