...
> **Related skills:** **component-system** for direct signal communication between components, **dependency-injection** for alternative decoupling approaches.
...
Create `res://autoloads/event_bus.gd`, then register it in **Project → Project Settings → Autoload** with the name `EventBus`.
...
Producers call `EventBus.<signal_name>.emit(...)`. The producer does not know which nodes are listening.
...
- [ ] Every consumer connects in `_ready()` and disconnects in `_exit_tree()`
