...
> See [references/mouse.md](references/mouse.md) for the full GDScript recipes (camera-look with sensitivity + invert toggle, mouse-mode switching, button events, custom cursor with shape variants).
...
> See [references/gamepad.md](references/gamepad.md) for the GDScript recipes (controller detection, deadzone analog reading, vibration via `start_joy_vibration`, detecting last-input-device for UI prompt swapping).
...
> See [references/touch.md](references/touch.md) for the GDScript basic touch event handling and the emulate-touch-from-mouse setting.
...
> See [references/action-rebinding.md](references/action-rebinding.md) for the full GDScript rebinding flow including ConfigFile save/load and the typical "press a key" capture UI.
