...
> **Related skills:** **gdscript-advanced** for production-grade depth (performance idioms, metaprogramming, @tool lifecycle, profiler-driven idioms), **godot-code-review** for style rules and anti-patterns, **state-machine** for state patterns, **event-bus** for signal architecture.
...
> See [references/super-in-virtual-methods.md](references/super-in-virtual-methods.md) for the full pattern (problem / fix) and a catalog of bugs from missing super calls.
...
The `@abstract` annotation prevents direct instantiation of a class and forces subclasses to implement any `@abstract`-annotated method.
