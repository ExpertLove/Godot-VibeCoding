...
> See [references/sprite-animation.md](references/sprite-animation.md) for the full GDScript example (a CharacterBody2D walking with `AnimatedSprite2D` driven by `Input.get_vector` and `flip_h`).
...
> See [references/skeleton-modifiers.md](references/skeleton-modifiers.md) for the full GDScript example with angle limits and influence blending.
...
> See [references/bone-constraints.md](references/bone-constraints.md) for the full deep dive — modifier table, scene structure, GDScript examples for AimModifier3D and CopyTransformModifier3D.
...
> See [references/ik-recipes.md](references/ik-recipes.md) for the full GDScript recipes — two-bone arm reach with influence blending (CCDIK3D), foot placement on uneven terrain (FABRIK3D + raycast), and basic FABRIK arm IK setup.
...
> See [references/common-recipes.md](references/common-recipes.md) for the full GDScript code (Hit Flash, Attack Combo with combo-window buffering).
