...
`InventorySlot` is a lightweight object tracking an item reference and its quantity. Define it as an inner class on `Inventory`.
...
> See [references/equipment.md](references/equipment.md) for the full GDScript `Equipment` class with `EquipmentSlotType` enum, equip / unequip API, and stat aggregation.
...
> See [references/ui-binding.md](references/ui-binding.md) for the full GDScript slot widget (drag/drop, hover preview), inventory grid layout, and tooltip wiring.
...
> See [references/serialization.md](references/serialization.md) for the GDScript save/load implementation with `version` field and ConfigFile / JSON variants.
