...
> See [references/music-manager.md](references/music-manager.md) for the full GDScript autoload (crossfade, push/pop stack, current-track query).
...
> See [references/sfx-pooling.md](references/sfx-pooling.md) for the GDScript pooled player and a 2D positional variant (pool of `AudioStreamPlayer2D` nodes that follow a target).
...
> See [references/audio-settings.md](references/audio-settings.md) for the full settings menu wiring with persistence (GDScript).
