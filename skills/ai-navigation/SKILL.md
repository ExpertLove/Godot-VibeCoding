...
> See [references/steering-behaviors.md](references/steering-behaviors.md) for the full GDScript implementations of seek, flee, arrive (with deceleration ramp), and wander (with circle-projection jitter).
...
> See [references/patrol-patterns.md](references/patrol-patterns.md) for the full GDScript waypoint-chain patrol with wait-timer pauses.
...
> See [references/behavior-trees.md](references/behavior-trees.md) for the full lightweight BT implementation (BTNode base + Sequence / Selector / Action) and a worked enemy that uses "chase OR patrol", in GDScript.
...
> See [references/chase-attack.md](references/chase-attack.md) for the full GDScript implementation (PATROL → CHASE → ATTACK transitions, attack cooldown timer, patrol-waypoint advancement, escape-range hand-off back to patrol).
