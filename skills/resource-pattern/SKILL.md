...
> Resources are editable directly in the Inspector, shareable across scenes, and can be created via **right-click → New Resource** in the FileSystem panel.
...
> See [references/editor-integration.md](references/editor-integration.md) for the full GDScript pattern with `class_name`, `@icon`, `@export_group`.
...
> See [references/configuration-pattern.md](references/configuration-pattern.md) for a worked LootTable + DropEntry example (GDScript).
...
> See [references/saving-resources.md](references/saving-resources.md) for the full save/load pattern (GDScript) and security caveat (never load `.tres` from untrusted sources — they execute embedded GDScript).
...
- [ ] Resource classes use `class_name` so the editor recognizes them for **New Resource**
