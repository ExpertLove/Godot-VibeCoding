...
Reference for `skills/particles-vfx/SKILL.md` — trigger modes, scene setup, limitations. **GDScript parity** (v1.6.0).
...
> Most projects configure subemitters in the Inspector (drag the child node onto the `SubEmitterNode` slot of the `ParticleProcessMaterial`). The GDScript code above is only needed when building particle effects programmatically.
