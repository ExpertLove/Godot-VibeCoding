...
> **Related skills:** **resource-pattern** for custom Resource editors, **godot-ui** for editor panel UI, , the script only runs when the game is playing.
...
> GDScript `@tool` scripts are excluded from exports automatically.

**Key rules:**
- Add `@tool` at the top of every script that needs editor access.
...
| `script` | The GDScript script resource |
...
> See [references/inspector-plugins.md](references/inspector-plugins.md) for the full GDScript scaffold (custom inspector + EditorProperty + registration boilerplate).
...
> See [references/dock-panels.md](references/dock-panels.md) for the full GDScript dock scaffold.
...
> See [references/inspector-plugins.md](references/inspector-plugins.md) for the full GDScript `EditorResourcePicker` and `EditorResourcePreviewGenerator` scaffolds.
...
> See [references/gizmos-deep-dive.md](references/gizmos-deep-dive.md) for the full GDScript gizmo plugin (with undo/redo wiring for handle commits).
...
- [ ] Main script extends `EditorPlugin` and is decorated with `@tool`
