...
`@rpc` marks a method as callable across the network. Choose the mode and transfer settings carefully — they affect both security and performance.
...
> See [references/spawning-networked-objects.md](references/spawning-networked-objects.md) for `MultiplayerSpawner` scene setup and the spawn-on-server flow (GDScript).
...
> See [references/player-join-flow.md](references/player-join-flow.md) for the full GDScript implementation (peer-connected handler, slot allocation, lobby state, gameplay transition).
...
> See [references/disconnect-handling.md](references/disconnect-handling.md) for the timeout detection settings, server-side cleanup, and client-side reconnect flow (GDScript).
